...
import de.osca.fama.digitaltwin.json
...
class HomeAssistantAdapter: SmartHomeAdapter {
    ...
    private fun createSensor(sensor: Sensor) {
        val deviceClass = HomeAssistantDeviceClass.entries.find {
            it.name == sensor.sensorType.name
        }

        val icon = when(sensor.sensorType.type) {
        ...
        val homeAssistantPayload = HomeAssistantPayload(
    ...
    companion object {
        ...
        private fun topic(component: HomeAssistantComponent, objectId: String, type: HomeAssistantTopicType) = "$discoveryPrefix/${component.name.lowercase()}/$objectId/${type.name.lowercase()}"
    }
}
