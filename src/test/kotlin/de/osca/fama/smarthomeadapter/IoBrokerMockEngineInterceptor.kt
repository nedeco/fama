...
class IoBrokerMockEngineInterceptor: KoinComponent {
    ...
    val famaFolderUrl = "${settings.IO_BROKER_URL}${IoBrokerApiCommandType.OBJECT.name.lowercase()}/${settings.IO_BROKER_PREFIX}"
    val stationFolderUrl = "${settings.IO_BROKER_URL}${IoBrokerApiCommandType.OBJECT.name.lowercase()}/${settings.IO_BROKER_PREFIX}.${settings.IO_BROKER_STATION_FOLDER_PREFIX}"
    ...
    val mockEngine: MockEngine = MockEngine { request ->
        ...
        interceptRequest(request)?.let {
            return@MockEngine respond(it.first ?: "", it.second, headers = headersOf("Content-Type" to listOf(ContentType.Application.Json.toString())))
        }
        when (request.url.toString()) {
            ...
            stationUrl(sensor?.station?.objectId ?: "") -> {
                if (request.method == HttpMethod.Get || request.method == HttpMethod.Post) {
                    ...
                    return@MockEngine respond(IoBrokerAdapter.json.encodeToString(stationPayload), HttpStatusCode.OK, headers = headersOf("Content-Type" to listOf(ContentType.Application.Json.toString())))
                }
            }
            sensorUrl(sensor?.station?.objectId ?: "", sensor?.objectId ?: "") -> {
                if (request.method == HttpMethod.Get || request.method == HttpMethod.Post) {
                    ...
                    return@MockEngine respond(IoBrokerAdapter.json.encodeToString(sensorPayload), HttpStatusCode.OK, headers = headersOf("Content-Type" to listOf(ContentType.Application.Json.toString())))
                }
            }
    ...
    fun sensorUrl(stationId: String, sensorId: String) = "$stationFolderUrl.$stationId.$sensorId"
    fun sensorStateUrl(stationId: String, sensorId: String) = "${settings.IO_BROKER_URL}${IoBrokerApiCommandType.STATE.name.lowercase()}/${settings.IO_BROKER_PREFIX}.${settings.IO_BROKER_STATION_FOLDER_PREFIX}.$stationId.$sensorId"
}
