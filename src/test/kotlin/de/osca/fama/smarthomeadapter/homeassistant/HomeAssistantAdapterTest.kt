...
class HomeAssistantAdapterTest: KoinTest {
    ...
    private val mqttManager: HomeAssistantMockMqttManager
    private val homeAssistantAdapter: HomeAssistantAdapter
    ...
    init {
        ...
        mqttManager = HomeAssistantMockMqttManager()
        loadKoinModules(getMockedFamaModule(mqttManager = mqttManager))
    ...
    fun testSensorCreation() = runBlocking {
        val homeAssistantPayload =
            HomeAssistantPayload(
                ...
                deviceClass = HomeAssistantDeviceClass.entries.find {
                    it.name == testSensor.sensorType.name
                },
                stateTopic = homeAssistantAdapter.topic(HomeAssistantComponent.SENSOR, testSensor.objectId, HomeAssistantTopicType.STATE),
                device =
                    HomeAssistantDevice(
        ...
        assert(mqttManager.messages[0].first == homeAssistantAdapter.topic(
    ...
    fun testSensorStateCreation() = runBlocking {
        homeAssistantAdapter.updateSensorStation(testSensor)
        assert(mqttManager.messages[1].first == homeAssistantAdapter.topic(
            HomeAssistantComponent.SENSOR,
            testSensor.objectId,
